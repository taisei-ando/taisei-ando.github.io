---
title: Publications
cms_exclude: true
type: landing

design:
  spacing: '4rem'

sections:
  - block: markdown
    content:
      title: International Publications
      text: |-
        ### 2026

        **CubeDVO: Cubemap-Spherical Deep Visual Odometry for a Monocular 360-Degree Camera**  
        **Taisei Ando**, Junwoon Lee, Takuya Igaue, Kohtaro Nakamura, Mitsuru Shinozaki, Toshihiro Kitajima, Qi An, and Atsushi Yamashita  
        IEEE Robotics and Automation Letters, Vol. 11, 2026. Accepted.  
        `JOURNAL` `IEEE RA-L`

        ### 2025

        **Efficient Distortion Mitigation in Equirectangular Images for Two-View Pose Estimation**  
        **Taisei Ando**, Junwoon Lee, Mitsuru Shinozaki, Toshihiro Kitajima, Qi An, and Atsushi Yamashita  
        International Journal of Automation Technology, Vol. 19, No. 3, pp. 226-236, May 2025.  
        [Paper](https://www.fujipress.jp/ijat/au/ijate001900030226/)  
        `JOURNAL` `IJAT`

        **Self-TIO: Thermal-Inertial Odometry via Self-Supervised 16-Bit Feature Extractor and Tracker**  
        Junwoon Lee, **Taisei Ando**, Mitsuru Shinozaki, Toshihiro Kitajima, Qi An, and Atsushi Yamashita  
        IEEE Robotics and Automation Letters, Vol. 10, No. 2, pp. 1003-1010, Feb. 2025.  
        [Paper](https://doi.org/10.1109/LRA.2024.3518303)  
        `JOURNAL` `IEEE RA-L`

        ### 2024

        **Highly Accurate and Fast Two-View Pose Estimation by Fast Reduction of Spherical Image Distortion Effects**  
        **Taisei Ando**, Junwoon Lee, Mitsuru Shinozaki, Toshihiro Kitajima, Qi An, and Atsushi Yamashita  
        2024 24th International Conference on Control, Automation and Systems (ICCAS), pp. 774-779, Oct. 2024.  
        `CONFERENCE` `ICCAS 2024`

        **TC-LTIO: Tightly-Coupled LiDAR Thermal Inertial Odometry for LiDAR and Visual Odometry Degraded Environments**  
        Junwoon Lee, **Taisei Ando**, Mitsuru Shinozaki, Toshihiro Kitajima, Qi An, and Atsushi Yamashita  
        2024 24th International Conference on Control, Automation and Systems (ICCAS), pp. 655-660, Oct. 2024.  
        [Paper](https://doi.org/10.23919/ICCAS63016.2024.10773102)  
        `CONFERENCE` `ICCAS 2024`
    design:
      columns: '1'

  - block: markdown
    content:
      title: Domestic Conference Presentations
      text: |-
        ### 2025

        **全天球カメラを用いた大きな姿勢変化に頑健な学習ベース視覚オドメトリ**  
        **安藤 大生**, Lee Junwoon, 伊賀上 卓也, 中村 浩太郎, 篠崎 充, 北島 利浩, 安 琪, 山下 淳  
        第26回計測自動制御学会システムインテグレーション部門講演会講演論文集, pp. 3011-3016, Dec. 2025.  
        `CONFERENCE` `SI 2025`

        **反射強度情報を活用した Transformer アテンションによる高精度な学習ベース点群マッチング**  
        山口 滉平, 伊賀上 卓也, Lee Junwoon, **安藤 大生**, 星井 智仁, 中村 浩太郎, 篠崎 充, 北島 利浩, 安 琪, 山下 淳  
        第26回計測自動制御学会システムインテグレーション部門講演会講演論文集, pp. 736-741, Dec. 2025.  
        `CONFERENCE` `SI 2025`

        ### 2024

        **全天球画像の2視点間位置姿勢推定のための適応的閾値を用いた相互最近傍マッチング**  
        **安藤 大生**, 小松 廉, Lee Junwoon, 篠崎 充, 北島 利浩, 淺間 一, 安 琪, 山下 淳  
        2024年度精密工学会春季大会学術講演会講演論文集, pp. 496-497, Mar. 2024.  
        `CONFERENCE` `JSPE 2024`

        ### 2023

        **適応的閾値を用いた相互最近傍マッチングによる全天球画像の2視点間位置姿勢推定精度向上**  
        **安藤 大生**, 小松 廉, Lee Junwoon, 篠崎 充, 北島 利浩, 淺間 一, 安 琪, 山下 淳  
        第24回計測自動制御学会システムインテグレーション部門講演会講演論文集, pp. 2448-2453, Dec. 2023.  
        `CONFERENCE` `SI 2023`
    design:
      columns: '1'
---
